...
describe('Node Server Request Listener Function', function () {
  ...
  // Test 1
  it('Should return the last "POST" message sent as first message', function () {
    var stubMsg = {
      username: 'Jono',
      text: 'Do my bidding!'
    };
    ...
    var messages = JSON.parse(res._data).results;
    expect(messages.length).to.be.above(0);
    expect(messages[0].username).to.equal('MatchewXD');
    expect(messages[0].text).to.equal('This is EPIC!');
    expect(res._ended).to.equal(true);
  });

  // Test 2
  it('Should check for options', function () {
  ...
  // Test 3
  it('Check if the server returns messages with a "message_id"', function () {
    var stubMsg = {
      username: 'Jono',
      text: 'Do my bidding!'
    };
    ...
    var messages = JSON.parse(res._data).results;
    expect(messages[0]['message_id']).to.equal(4);
  });
